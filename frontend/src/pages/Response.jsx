...
function Response({ prompt, data }) {
  return (
    <section className="relative flex flex-col items-center justify-start min-h-screen mt-[8rem]">
      <div className="min-h-[600px] min-w-[1300px] response-content rounded-[1rem] pt-[3rem] pb-[5rem] px-[6rem] mt-[5rem] flex flex-col items-center justify-center">
        <h1 className="text-5xl mb-[3rem] font-medium ">"{prompt ? prompt : "No Prompt"}"</h1>
        {data?.map((suggestion, key) => (
          <p
            key={key}
            className="response-text w-[100%] max-w-[1200px] mt-[0rem] text-2xl text-center leading-10 max-h-[600px] overflow-scroll"
          >
            {suggestion}
          </p>
        ))}
